...
For each observation, the Cook's distance was used to investigated the problem of influential points. Points with a Cook's distance larger than 1 are considered to be influence points.
...
```{r}
summary(lm(expend ~ employ + lawyers + bad + crime, data=expenses))
summary(lm(expend ~ employ + lawyers + bad + pop, data=expenses))
```

The variable pop is added to the model with
...
```{r}
step_up = (lm(expend ~ employ + lawyers + bad + pop + crime, data=expenses))
summary(step_up)
```

The final step-up model contains all explanatory variables. However, when there is collinearity among the explanatory variables, one should actually avoid having two collinear explanatory variables in the model. This is something to keep in mind when tweaking the model further.
...
The model coefficients of the relevant variables selected by the LASSO model are shown below:

```{r}
lambda_min = cv_lasso$lambda_min
lambda_1se = cv_lasso$lambda.1se
coef(lasso_mod, s=cv_lasso$lambda_min)
```
...
```{r}
y.pred=predict(lasso_mod, s=lambda_min, newx = x.test) 
